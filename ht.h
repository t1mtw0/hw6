...
template <typename KeyType> struct Prober {
    // Data members
    ...
    static const HASH_INDEX_T npos =
        (HASH_INDEX_T)-1; // used to indicate probing failed
    void init(HASH_INDEX_T start, HASH_INDEX_T m, const KeyType &key) {
        (void)key; // avoid unused argument warnings since base class doesn't
                   // use key
        start_ = start;
        m_ = m;
        numProbes_ = 0;
...
template <typename KeyType> struct LinearProber : public Prober<KeyType> {
...
struct DoubleHashProber : public Prober<KeyType> {
  ...
  private:
    ...
    HASH_INDEX_T findModulusToUseFromTableSize(HASH_INDEX_T currTableSize) {
        ...
        for (int i = 0; i < DOUBLE_HASH_MOD_SIZE &&
                        DOUBLE_HASH_MOD_VALUES[i] < currTableSize;
             i++) {
  ...
  public:
     ...
     *
     *
     * @param h2  h2(k) - Object with an operator()(const KeyType&) defined for
     * it
     */
...
template <typename K, typename V, typename Prober = LinearProber<K>,
          typename Hash = std::hash<K>, typename KEqual = std::equal_to<K>>
class HashTable {
  public:
    typedef K KeyType;
    typedef V ValueType;
     ...
     *
     * @param resizeAlpha Loading factor threshold at which the table should
     * resize
     * @param prober Probing object of type Prober
     * @param hash Hash functor that supports hash(key) and returns a
     * HASH_INDEX_T
     * @param kequal Functor that checks equality of two KeyType objects
     */
    HashTable(double resizeAlpha = 0.4, const Prober &prober = Prober(),
              const Hasher &hash = Hasher(), const KEqual &kequal = KEqual());
  ...
  private:
    ...
    KEqual kequal_;
    mutable Prober
        prober_; // mutable allows const member functions to modify this member
    // debug/performance counters
    mutable size_t totalProbes_; // mutable allows const member functions to
                                 // modify this member
...
template <typename K, typename V, typename Prober, typename Hash,
          typename KEqual>
HashTable<K, V, Prober, Hash, KEqual>::HashTable(double resizeAlpha,
                                                 const Prober &prober,
                                                 const Hasher &hash,
                                                 const KEqual &kequal)
...
void HashTable<K, V, Prober, Hash, KEqual>::insert(const ItemType &p) {
    if (size() + 1 >= std::floor(CAPACITIES[mIndex_] * resizeAlpha_))
        resize();
...
void HashTable<K, V, Prober, Hash, KEqual>::resize() {
    if (++mIndex_ >= 28)
        throw std::logic_error("Cannot resize again.");
...
template <typename K, typename V, typename Prober, typename Hash,
          typename KEqual>
HASH_INDEX_T
HashTable<K, V, Prober, Hash, KEqual>::probe(const KeyType &key) const {
...
void HashTable<K, V, Prober, Hash, KEqual>::reportAll(std::ostream &out) const {
    for (HASH_INDEX_T i = 0; i < CAPACITIES[mIndex_]; ++i) {
        if (table_[i] != nullptr) {
            out << "Bucket " << i << ": " << table_[i]->item.first << " "
